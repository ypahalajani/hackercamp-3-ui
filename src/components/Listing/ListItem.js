...
import { FlexContainer, Element, ElementBar } from 'components/Listing/components';
...
export const isObjectEmpty = obj => typeof obj === 'object' && Object.keys(obj).length === 0;
		...
		return (
			<Container>
				<IconWrapper style={{ margin: '2px 12px 0 0' }}>
					{!isObjectEmpty(issue) &&
					issue.iconUrl && <img src={issue.iconUrl} alt={issue.type} style={{ width: 20 }} />}
				</IconWrapper>
				<FlexContainer column style={{ width: '90%' }}>
					...
					<TagsContainer alignItems='center' style={{ flexWrap: 'wrap' }}>
						...
						{!isObjectEmpty(priority) &&
						priority.name && (
							<Element title='Priority'>
								<FlexContainer alignItems='center'>
									<img src={priority.iconUrl} style={{ width: 11, margin: '0 4px 0 0' }} />
									{priority.name}
								</FlexContainer>
							</Element>
						)}
											...
											<FlexContainer alignItems='center'>
												<img
													src={
														...
														'https://innovaccer.atlassian.net/secure/projectavatar?size=xsmall&s=xsmall&pid=11915&avatarId=11438' ? (
															`${process.env.PUBLIC_URL}/img/alt.png`
														) : (
															project.avatarUrls['16x16']
														)
													}
